...
public class SpringBeanInjector implements BeanInjector{
	...
	private void initPostProcessor() {
		...
		String[] pps = this.cBeanFactory
				.getBeanNamesForType(PropertyPlaceholderConfigurer.class);
		if ((pps != null) && (pps.length > 0)) {
			PropertyPlaceholderConfigurer ppc = (PropertyPlaceholderConfigurer) this.cBeanFactory
					.getBean(pps[0]);
	...
	@Override
	public void registClass(String name, Class<?> clazz,
		...
		if (!consParams.isEmpty()) {
			...
			for (String ck : cKeys) {
				...
				if (prix > 0) {
					...
					rbd.getConstructorArgumentValues().addIndexedArgumentValue(
							conIndex, tValue);
				} else {
					rbd.getConstructorArgumentValues().addGenericArgumentValue(
							tValue);
				}
			}
		}
	...
	public <T> T getBeanOnSafe(Class<T> type) {
		...
		if (names.length > 1) {
			...
			throw new IllegalArgumentException(
					"No unique bean of type [com.mce.core.bean.TestABean] is defined: expected single bean but found "
							+ count);
		}
